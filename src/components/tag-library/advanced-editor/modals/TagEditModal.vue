  ...
  setup(props, { emit }) {
    ...
    watch(() => props.tag, (newTag) => {
      form.value = {
        ...
        subTitles: newTag.subTitles || ''
      };
    }, { immediate: true });
    
    // 保存标签
    const save = () => {
      console.log('TagEditModal - 保存按钮点击，表单数据:', form.value);
      
      if (!form.value.content.trim()) {
        console.error('TagEditModal - 标签内容为空，不执行保存');
        return;
      }
      ...
      const tagData = {
        ...
        subTitles: form.value.subTitles.trim()
      };
      
      console.log('TagEditModal - 发送保存事件，数据:', tagData);
      emit('save', tagData);
    };
